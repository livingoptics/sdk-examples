"""# Spectrally enhanced object detection with LOCamera

The addition of spectral information on top of third party
object detectors such as [YOLO](https://github.com/ultralytics/ultralytics) can make detections more reliable and
allows for distinguishing between real and fake versions of objects. For example, 'face spoofing' can easily be
detected by comparing the spectral signal of real faces to the spectra of detected face in a scene and rejecting any
which don't have sufficiently face like spectra.

This example shows how to use the LOCamera for spectrally enhanced face detection, but changing the input parameters to
`get_from_huggingface_model()` allows you to adapt it to detect other object types.

Commandline:
    `python object_detection_plus.py`
...
def main():
    calibrationFolder = Path("/datastore/lo/share/calibrations/latest_calibration").as_posix()
    model = get_from_huggingface_model(
        repo_id="jaredthejelly/yolov8s-face-detection", filename="YOLOv8-face-detection.pt"
    )
